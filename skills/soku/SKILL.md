...
Code, Codex, Cursor, or any terminal. It talks to Soku over `/api/cli/*`; the
CLI path needs no MCP host. Treat this file as the router. Load the relevant
reference file before acting on a detailed workflow. If you have no usable
shell, skip to "No Shell? Use the Soku MCP Connection".
